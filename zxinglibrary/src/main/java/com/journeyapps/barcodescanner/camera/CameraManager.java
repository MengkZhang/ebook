...
import com.journeyapps.barcodescanner.Utils;
...
public final class CameraManager {
    ...
    //    private AutoMoveFocusManager focusManager;
...
//    }
    ...
    public void startPreview() {
        ...
        if (theCamera != null && !previewing) {
            ...
            previewing = true;
//            focusManager = new AutoMoveFocusManager(context, camera, settings);
//            start();
...
//        stop();
    ...
    private void setDesiredParameters(boolean safeMode) {
        ...
        try {
            if (null != getPreviewSize()) {
                Point point = new Point(getPreviewSize().width / 2, getPreviewSize().height / 2);
        ...
        if (parameters.isZoomSupported()) {
            ...
            parameters.setZoom(parameters.getMaxZoom() / 10 + 2);
        }
        try {
            Camera.Size mSize = findFitPreResolution(camera);
            Camera.Parameters mParameters = camera.getParameters();
            mParameters.setPictureSize(mSize.width, mSize.height);
        } catch (Exception e) {
        }
