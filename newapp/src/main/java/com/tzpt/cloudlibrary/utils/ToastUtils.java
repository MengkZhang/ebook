...
//        return getSingleToast(Utils.getContext().getResources().getText(resId).toString(), duration);
        return getSingleToast(CloudLibraryApplication.getAppContext().getResources().getText(resId).toString(), duration);
    }

    private static Toast getSingleToast(String text, int duration) {
        if (mToast == null) {
//            mToast = Toast.makeText(Utils.getContext(), text, duration);
            mToast = Toast.makeText(CloudLibraryApplication.getAppContext(), text, duration);
        } else {
            mToast.setText(text);
        }
        return mToast;
...
//        return getToast(Utils.getContext().getResources().getText(resId).toString(), duration);
        return getToast(CloudLibraryApplication.getAppContext().getResources().getText(resId).toString(), duration);
    }

    private static Toast getToast(String text, int duration) {
//        return Toast.makeText(Utils.getContext(), text, duration);
        return Toast.makeText(CloudLibraryApplication.getAppContext(), text, duration);
    }
}
