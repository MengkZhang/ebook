...
public class SwipeBackHelper extends Handler {
    ...
    private boolean mIsSupportSlideBack;                //是否支持侧滑
    private int mTouchSlop;
    private boolean mIsInThresholdArea;                 //是否满足滑动区域做处理
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_ACTION_DOWN: //点击事件
                // hide input method 按下关闭键盘
            ...
            case MSG_SLIDE_CANCEL://开始滑动，不返回前一个页面
                startSlideAnim(true);
                break;

            case MSG_SLIDE_CANCELED: //结束滑动，不返回前一个页面
                mDistanceX = 0;
                mIsSliding = false;
                mViewManager.removeShadowView();
                mViewManager.resetPreviousView();
            ...
            case MSG_SLIDE_PROCEED://开始滑动，返回前一个页面
                startSlideAnim(false);
                break;

            case MSG_SLIDE_FINISHED://结束滑动，返回前一个页面
                mViewManager.addCacheView();
                mViewManager.removeShadowView();
                mViewManager.resetPreviousView();
    ...
    private class ViewManager {
         ...
         *
         * @return Is view added successfully 界面是否添加成功
         */
