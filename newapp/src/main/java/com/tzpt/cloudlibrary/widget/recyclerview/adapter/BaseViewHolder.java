...
import com.tzpt.cloudlibrary.base.adapter.glide.GlideCircleTransform;
...
public class BaseViewHolder<M> extends RecyclerView.ViewHolder {
    ...
    public BaseViewHolder setTextSize(int viewId, float value) {
        TextView view = getView(viewId);
    ...
    public BaseViewHolder setImageUrl(int viewId, int errorId, String imgUrl) {
        ImageView view = getView(viewId);
        GlideApp.with(mContext).load(imgUrl).error(errorId).centerCrop().into(view);
        return this;
    }

    public BaseViewHolder setImageUrl(int viewId, String imgUrl) {
        ImageView view = getView(viewId);
        GlideApp.with(mContext).load(imgUrl).centerCrop().into(view);
        return this;
    }

    public BaseViewHolder setImageUrl(int viewId, String imgUrl, int placeHolderRes, int errorRes) {
        ImageView view = getView(viewId);
        GlideApp.with(mContext).load(imgUrl).placeholder(placeHolderRes).error(errorRes).centerCrop().into(view);
        return this;
    }

    public BaseViewHolder setHorizontalImageUrl(int viewId, String imgUrl) {
        ImageView view = getView(viewId);
        GlideApp.with(mContext).load(imgUrl).error(R.mipmap.ic_video_error_image).centerCrop().into(view);
        return this;
    }

    public BaseViewHolder setImageUrl(int viewId, String imgUrl, int placeHolderRes) {
        ImageView view = getView(viewId);
        GlideApp.with(mContext).load(imgUrl).placeholder(placeHolderRes).error(R.mipmap.ic_nopicture).centerCrop().into(view);
        return this;
    }

    public BaseViewHolder setCircleImageUrl(int viewId, String imgUrl, int placeHolderRes) {
        ImageView view = getView(viewId);
        GlideApp.with(mContext).load(imgUrl).placeholder(placeHolderRes).transform(new GlideCircleTransform()).into(view);
        return this;
    }

    public BaseViewHolder setRoundImageUrl(int viewId, String imgUrl, int placeHolderRes, int errorRes, float radius) {
        ImageView view = getView(viewId);
        GlideApp.with(mContext)
                .load(imgUrl)
                .placeholder(placeHolderRes)
                .error(errorRes)
                .transform(new RoundedCornersTransformation(radius, RoundedCornersTransformation.CornerType.ALL))
                .into(view);
        return this;
    }
    ...
    public BaseViewHolder setCompoundLeftDrawables(int viewId, Drawable drawable) {
        TextView view = getView(viewId);
        view.setCompoundDrawablesWithIntrinsicBounds(drawable, null, null, null);
        return this;
    }

    public BaseViewHolder setCompoundRightDrawables(int viewId, Drawable drawable) {
        TextView view = getView(viewId);
