package com.tzpt.cloudlibrary.widget.pullrefreshlayout;
...
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.api.DefaultRefreshFooterCreator;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.api.DefaultRefreshHeaderCreator;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.api.RefreshContent;
...
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.api.RefreshInternal;
...
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.constant.DimensionStatus;
...
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.footer.ClassicsFooter;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.header.ClassicsHeader;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.impl.RefreshContentWrapper;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.impl.RefreshFooterWrapper;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.impl.RefreshHeaderWrapper;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.listener.OnLoadMoreListener;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.listener.OnMultiPurposeListener;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.listener.OnRefreshListener;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.listener.OnRefreshLoadMoreListener;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.listener.OnStateChangedListener;
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.util.DelayedRunnable;
...
import com.tzpt.cloudlibrary.widget.pullrefreshlayout.util.ViscousFluidInterpolator;
...
import static com.tzpt.cloudlibrary.widget.pullrefreshlayout.util.DensityUtil.dp2px;
import static com.tzpt.cloudlibrary.widget.pullrefreshlayout.util.SmartUtil.fling;
import static com.tzpt.cloudlibrary.widget.pullrefreshlayout.util.SmartUtil.getColor;
import static com.tzpt.cloudlibrary.widget.pullrefreshlayout.util.SmartUtil.isScrollableView;
...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent, NestedScrollingChild {
    ...
    protected float mDragRate = .5f;            //拖动比率
    ...
    protected Scroller mScroller;               //处理滚动效果的工具类
    protected VelocityTracker mVelocityTracker; //速度跟踪
    protected Interpolator mReboundInterpolator;//插值器

    //</editor-fold>

    //<editor-fold desc="功能属性">
    protected int[] mPrimaryColors;                                 //颜色数组
    protected boolean mEnableRefresh = true;                        //是否可用刷新
    protected boolean mEnableLoadMore = false;                      //是否可用加载更多
    ...
    protected OnRefreshListener mRefreshListener;                   //刷新监听
    protected OnLoadMoreListener mLoadMoreListener;                 //加载更多监听
    protected OnMultiPurposeListener mOnMultiPurposeListener;       //拉动监听
    protected ScrollBoundaryDecider mScrollBoundaryDecider;         //滚动监听
    //</editor-fold>
    ...
    protected static DefaultRefreshFooterCreator sFooterCreator = new DefaultRefreshFooterCreator() {
        @NonNull
        @Override
        public RefreshFooter createRefreshFooter(@NonNull Context context, @NonNull RefreshLayout layout) {//默认底部视图构造器
    ...
    protected static DefaultRefreshHeaderCreator sHeaderCreator = new DefaultRefreshHeaderCreator() {
        @NonNull
        @Override
        public RefreshHeader createRefreshHeader(@NonNull Context context, @NonNull RefreshLayout layout) {//默认顶部视图构造器
    ...
    public SmartRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        super.setClipToPadding(false);  //控件的绘制区域是否在padding里面

        DensityUtil density = new DensityUtil();
        ViewConfiguration configuration = ViewConfiguration.get(context);   //系统UI中所使用到的标准常量类
        //滑动属性实例化
        mScroller = new Scroller(context);              //滚动控制器
        mVelocityTracker = VelocityTracker.obtain();    //速度跟踪
        mScreenHeightPixels = context.getResources().getDisplayMetrics().heightPixels;//屏幕高度
        mReboundInterpolator = new ViscousFluidInterpolator();  //插值器
        ...
        int accentColor = ta.getColor(R.styleable.SmartRefreshLayout_srlAccentColor, 0);    //页面背景色
        int primaryColor = ta.getColor(R.styleable.SmartRefreshLayout_srlPrimaryColor, 0);  //基色
        if (primaryColor != 0) {
            if (accentColor != 0) {
    ...
    //页面初始化后，配置headerView，footerView，contentView
    @Override
    protected void onFinishInflate() {
        super.onFinishInflate();
        final int count = super.getChildCount();//页面中的子view个数
        if (count > 3) {
            throw new RuntimeException("最多只支持3个子View，Most only support three sub view");
        }

        int contentLevel = 0;
        int indexContent = -1;//判断内容下标
        for (int i = 0; i < count; i++) {
            View view = super.getChildAt(i);
            if (isScrollableView(view) && (contentLevel < 2 || i == 1)) {       //是否可滑动的view
                indexContent = i;
                contentLevel = 2;
            } else if (!(view instanceof RefreshInternal) && contentLevel < 1) {//刷新内部组件 header，footer
                indexContent = i;
                contentLevel = i > 0 ? 1 : 0;
            }
        }

        int indexHeader = -1;//判断header下标
        int indexFooter = -1;//判断footer下标
        if (indexContent >= 0) {//如果当前有内容，则获取内容实例
            mRefreshContent = new RefreshContentWrapper(super.getChildAt(indexContent));
            if (indexContent == 1) {
                indexHeader = 0;
                if (count == 3) {
                    indexFooter = 2;//设置footer下标位置
                }
            } else if (count == 2) {//如果只有两个子view
                indexFooter = 1;    //设置footer下标位置
            }
        }
    ...
    protected void onAttachedToWindow() {
        ...
        if (!thisView.isInEditMode()) {//当前view 不可编辑
            ...
            if (mRefreshContent == null) {//创建出现错误后的刷新内容
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        for (int i = 0, len = super.getChildCount(); i < len; i++) {
            ...
            if (mRefreshHeader != null && mRefreshHeader.getView() == child) {
                ...
                if (mHeaderHeightStatus.gteReplaceWith(DimensionStatus.XmlLayoutUnNotify)) {//当前是在xml里面配置布局
                    heightSpec = makeMeasureSpec(Math.max(mHeaderHeight - lp.bottomMargin - lp.topMargin, 0), EXACTLY);
                    headerView.measure(widthSpec, heightSpec);//计算配置headerView 宽高
                } else if (mRefreshHeader.getSpinnerStyle() == SpinnerStyle.MatchLayout) { //填满布局
                    int headerHeight = 0;
                    if (!mHeaderHeightStatus.notified) {//未通知状态
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if ((finalY < 0 && (mEnableOverScrollDrag || isEnableRefresh()) && mRefreshContent.canRefresh())
                ...
                if (mVerticalPermit) {
                    float velocity;//速率
                    if (Build.VERSION.SDK_INT >= 14) {//计算滑动的速率
     ...
     * 设置多功能监听器
     *
     * @param listener 建议使用 {@link com.tzpt.cloudlibrary.widget.pullrefreshlayout.listener.SimpleMultiPurposeListener}
     * @return SmartRefreshLayout
     */
    @Override
     ...
     * 设置滚动边界
     *
     * @param boundary 建议使用 {@link com.tzpt.cloudlibrary.widget.pullrefreshlayout.impl.ScrollBoundaryDeciderAdapter}
     * @return SmartRefreshLayout
     */
    @Override
