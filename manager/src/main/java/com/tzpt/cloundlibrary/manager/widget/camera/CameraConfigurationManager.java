...
public class CameraConfigurationManager {
    ...
    private Camera.Size getSizeForPreviewSize(Camera.Parameters parameters) {
        ...
        int PreSize = 0;
        for (int i = ViewSize.size() - 2; i > 0; i--) {
            if (ViewSize.get(i).height <= screenResolution.x && screenResolution.x <= ViewSize.get(i + 1).height) {
                PreSize = i;
                break;
            }
        }
        if (PreSize > 0) {
            return ViewSize.get(PreSize);
        }
    ...
    public void initFromCameraParameters(Camera camera) {
        ...
        Camera.Size size = getLargePreviewSize(camera);
        if (size != null) {
